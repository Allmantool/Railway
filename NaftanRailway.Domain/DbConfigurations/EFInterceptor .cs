...
namespace NaftanRailway.Domain.DbConfigurations {
    ...
    public class EfInterceptor : IDbCommandInterceptor {
        ...
        /// When the application is compiled in the release configuration, the Debug elements will not be compiled into the code.
        /// As Debug.WriteLine writes to all the trace listeners in the Listeners collection, 
        /// it is possible that this could be output in more than one place (Visual Studio output window, Console, Log file, third-party application 
        ...
        ///The difference between Debug and Trace is all of the methods in Debug only write out when the DEBUG compilation symbol is set(default on for debug, 
        /// off for release) when the symbol is not set the methods are never called in your code.Trace looks for the TRACE symbol (default on for both debug and release). 
        /// Other that that, the two classes are identical.In fact if you modify Debug.Listeners to add a new listener it will also modify Trace.Listeners as both just point to 
        /// the internal static property TraceInternal.Listeners
        ///As for picking which one to use, Do you want diagnostic information to show up in release and debug mode? use Trace, Debug only? use Debug. 
        /// Do you want it to be visible to a end user without a debugger attached? use Console or add a console trace listener.
        /// </summary>
